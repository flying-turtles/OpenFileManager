...
pub async fn start_project_transfer(
    ...
    // Build connected device map with drive_speed info
    let volumes = devices::detect_volumes();
    let all_devices = db::get_all_devices(&state.pool).await?;
    let volume_ids: std::collections::HashSet<String> = volumes.iter().map(|v| v.id.clone()).collect();

    let mut connected_devices: HashMap<String, StorageDevice> = HashMap::new();
    for mut dev in all_devices {
        let is_connected = volume_ids.contains(&dev.id)
            || std::path::Path::new(&dev.mount_point).exists();
        if is_connected {
            dev.is_connected = true;
            // Update mount_point from detected volume if available
    ...
    let (check, resolved) = transfer::check_and_resolve(&files, &connected_devices, &target_device_id);

    let _ = on_event.send(TransferEvent::CheckComplete(check.clone()));

    if !check.unavailable_files.is_empty() && !skip_unavailable {
        return Ok(());
    }
